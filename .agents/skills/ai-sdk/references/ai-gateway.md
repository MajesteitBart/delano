...
When multiple versions of a model exist, use the one with the highest version number (e.g., prefer `anthropic/claude-sonnet-5` over `anthropic/claude-sonnet-4.6` over `anthropic/claude-sonnet-4.5` over `anthropic/claude-sonnet-4`).

...
test("resolveBash skips an unusable Windows shim and selects working Git Bash", () => {
  ...
  assert.ok(calls.some(([command, arg]) => command === wslShim && arg === "--version"));
...
test("resolveBash reports every failed Windows candidate with its probe reason", () => {
  ...
  assert.ok(thrown.message.includes(wslShim));
...
test("resolveBash preserves POSIX discovery while capability-checking the result", () => {
  ...
  assert.deepEqual(calls.slice(0, 3).map(([command, arg]) => [command, arg]), [
    ["which", "-a"],
    [bashPath, "--version"],
    [bashPath, "-lc"]
  ]);

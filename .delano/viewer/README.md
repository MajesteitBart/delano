...
Project folders get a right-side outline for the spec, plan, decisions/progress, workstreams, and tasks. Opening a workstream shows its indexed tasks below the contract body with canonical status, dependency-aware availability, priority/estimate context, and direct task navigation. Viewer data tables provide sortable, per-column filterable headers, content-aware column widths, top-aligned rows, and pagination where needed. Context and template folders keep filters scoped to the roles/statuses that actually exist in the selected folder.

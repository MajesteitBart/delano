...
# Delivery Plan: Delano Trust and Safety Runtime
...
## Workstream Design
- `WS-A Privacy-Safe Logging Defaults`: Make prompt and hook logging opt-in, redacted, and safe to commit around.
- `WS-B Path and Hook Output Safety`: Prevent accidental absolute path leakage from hooks, scripts, and copied outputs.
- `WS-C Package Metadata Trust Gates`: Detect stale package metadata, manifest drift, missing payload files, and unexpected payload files.
- `WS-D Agent Entry Instructions`: Upgrade root and adapter-facing instructions into a compact command and boundary index.

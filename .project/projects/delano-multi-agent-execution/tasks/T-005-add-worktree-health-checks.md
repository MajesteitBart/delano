---
id: T-005
name: Add worktree health checks
status: done
workstream: WS-C
...
# Task: Add worktree health checks

## Description
Check dirty state, branch identity, stale worktrees, and risky shared files before parallel work starts.
...
- 2026-04-30T01:48:00Z: Inspected stream-aware task selection state after `2396d29`. Added `scripts/check-worktree-health.mjs` to report branch, upstream, dirty state, and git health before multi-agent execution; mirrored to `.agents` and `.claude`, wired `check:worktree-health` into PM validation and tests, and staged it in npm payload. Validation passed: `npm run check:worktree-health`, `bash .agents/scripts/pm/validate.sh`, and `npm test`.

...
- 2026-05-11: Refined Codex hook operator guidance to cover `codex --enable hooks`, `[features].hooks`, project trust, and hook trust prompts, and changed SessionStart context to a semicolon-separated sentence so it remains readable when Codex displays hook context inline. Validation passed: `npm run build:assets`; `npm test` (63/63); `npm run check:package-manifest`; `bash .agents/scripts/pm/validate.sh`.
- 2026-06-25: Bumped `@bvdm/delano` from `0.2.10` to `0.2.11`, rebuilt assets, confirmed `node bin/delano.js --version` returned `0.2.11`, and passed `bash .agents/scripts/pm/validate.sh --release` with 0 errors and 0 warnings after regenerating the Claude mirror. `npm test` still has a separate 2-test `import-spec-kit` Windows path-resolution failure.

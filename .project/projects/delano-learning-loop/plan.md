...
# Delivery Plan: Delano Learning Loop
...
## Workstream Design
- `WS-A Delivery Metrics`: Track task throughput, reopened work, validation failures, missing evidence, drift incidents, and blocked time.
- `WS-B Context Debt Scoring`: Audit context files for placeholders, staleness, missing commands, and applicability.
- `WS-C Skill and Output Evals`: Add eval fixtures for discovery, breakdown, execution, quality, and sync outputs.
- `WS-D Closeout-to-Rules Workflow`: Turn retrospectives into proposed updates without silently rewriting shared behavior.
